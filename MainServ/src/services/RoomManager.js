...
export class RoomManager {
  ...
  addPlayerToRoom(roomId, playerName, socketId) {
    ...
    if (existingRoomId) {
      if (existingRoomId === roomId) {
        ...
        if (existingPlayer) {
          console.log(`Joueur ${existingPlayer.name} (${existingPlayer.id}) déjà dans la salle ${roomId}`);
    ...
    console.log(`Joueur ${playerName} (${playerId}) ajouté à la salle ${roomId}`);
    return playerId;
  }
  ...
  removePlayerFromRoom(roomId, playerId) {
    ...
    if (room) {
      ...
      if (player) {
        console.log(`Joueur ${player.name} (${playerId}) retiré de la salle ${roomId}`);
